...
{
    /// <summary>
    /// Логика взаимодействия для MaterialToMaterialStockTable.xaml
    /// </summary>
    public partial class MaterialToMaterialStockTable : Page
    {
        DateTime? fromDate = null;
        DateTime? toDate = null;
        public static SuppliesMaterialsInMaterialStock selectedSupply;
        public static DataGrid tableLink;

        public MaterialToMaterialStockTable(DateTime? fromDate, DateTime? toDate)
        {
            InitializeComponent();
        ...
        private void addSupply(object sender, RoutedEventArgs e)
        {
            ...
            SuppliesMaterialsInMaterialStock supply = new SuppliesMaterialsInMaterialStock();
            supply.IDSupply = 0;
            supply.Date = DateTime.Now;
            EditWindows.WindowEditSupplyProductToProductStock windowEditSupplyProductToProductStock = new EditWindows.WindowEditSupplyProductToProductStock(supply);
            windowEditSupplyProductToProductStock.ShowDialog();
            InitializeTable(fromDate, toDate);
        }

        public void InitializeTable(DateTime? fromDate, DateTime? toDate)
        {
            if (DataBank.currentUser.Post == "бухгалтер склада материалов")
            {
                table.ItemsSource = null;
                if (fromDate == null && toDate == null)
                {
                    table.ItemsSource = MainWindow.shoesKursovoiEntities.SuppliesMaterialsInMaterialStock.ToList();
                }
            ...
            {
                MessageBox.Show("Вы не бухгалтер склада материалов");
            }
        ...
        private void btEditSupply(object sender, RoutedEventArgs e)
        {
            ...
            var supply = button.DataContext as SuppliesMaterialsInMaterialStock;
            EditWindows.WindowEditSupplyProductToProductStock windowEditSupplyProductToProductStock = new EditWindows.WindowEditSupplyProductToProductStock(supply);
            windowEditSupplyProductToProductStock.ShowDialog();
            InitializeTable(fromDate, toDate);
        ...
        private void deleteSupply(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (supply != null)
                {
                    var contain = supply.SuppliesMaterialsInMaterialStockContains.ToList();
                    for (int i = 0; i < contain.Count; i++)
                    {
                        contain[i].Material.CountInStock -= contain[i].CountOfMaterial;
                    }
                    MainWindow.shoesKursovoiEntities.SuppliesMaterialsInMaterialStock.Remove(supply);
                }
        ...
        {
            selectedSupply = table.SelectedItem as SuppliesMaterialsInMaterialStock;
            if (selectedSupply == null)
            {
                Pages.Supplies.lvMaterialLink.ItemsSource = null;
                table.SelectedItem = null;
                Pages.Supplies.lvMaterialLink.Visibility = Visibility.Collapsed;
            ...
            else
            {
                ...
                Pages.Supplies.lvMaterialLink.Visibility = Visibility.Visible;
                Pages.Supplies.lvMaterialLink.ItemsSource = null;
                Pages.Supplies.lvMaterialLink.ItemsSource = selectedSupply.SuppliesMaterialsInMaterialStockContains.ToList();
            }
        }
        ...
        private void btWord(object sender, RoutedEventArgs e)
        {
            int resultCount = 0;
            decimal allCost = 0;
            var supplyContainList = selectedSupply.SuppliesMaterialsInMaterialStockContains.ToList();
            ...
            for (int i = 0; i < supplyContainList.Count; i++)
            {
                ...
                cellRange.Text = supplyContainList[i].Material.Name;
                cellRange = supplyContainTable.Cell(i + 2, 3).Range;
                cellRange.Text = supplyContainList[i].CountOfMaterial.ToString();
                cellRange = supplyContainTable.Cell(i + 2, 4).Range;
                cellRange.Text = supplyContainList[i].Material.CostWithNDS.ToString();
                cellRange = supplyContainTable.Cell(i + 2, 5).Range;
                cellRange.Text = (supplyContainList[i].Material.CostWithNDS * supplyContainList[i].CountOfMaterial).ToString();
                resultCount += supplyContainList[i].CountOfMaterial;
                allCost += supplyContainList[i].Material.CostWithNDS * supplyContainList[i].CountOfMaterial;
            }
